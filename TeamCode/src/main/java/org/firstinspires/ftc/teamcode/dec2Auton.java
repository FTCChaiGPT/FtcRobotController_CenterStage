...
@Autonomous(name="PPKAutonBlueBackstage_Park_IN", group = "Auto")
public class dec2Auton extends LinearOpMode {
...
//
//            Scalar lowerBlue = new Scalar(90, 50, 50); //old values were 110, 150, 50
//            Scalar upperBlue = new Scalar(140,255,255); //old values were 140, 255,255
...
//        }
        ...
        public Mat processFrame(Mat input, int hueStart, int hueEnd, int saturationStart, int valueStart) {
            if (!startProcessing) {
                return input;
            }
            lastDetectedPosition = PropPostition.UNKNOWN;
            ...
            Scalar lowerBlue = new Scalar(hueStart, saturationStart, valueStart); // Dynamic HSV lower bound
            Scalar upperBlue = new Scalar(hueEnd, 255, 255); // Dynamic HSV upper bound
            Mat propMask = new Mat();
            Core.inRange(hsvImage, lowerBlue, upperBlue, propMask);

            List<org.opencv.core.MatOfPoint> contours = new ArrayList<>();
            Mat hierarchy = new Mat();
            Imgproc.findContours(propMask, contours, hierarchy, Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
    ...
    private void DropPurplePixel(PropPostition spikePosition) {
        ...
        switch (spikePosition1) {
            case RIGHT:
                ...
                telemetry.update();
                forward(28, motorPowerValue);
                turn_Right(95, motorPowerValue);
            ...
            case LEFT:
                //right logic
                telemetry.addLine("LEFT, going for RIGHT purple pixel drop");
                telemetry.update();
                forward(5, motorPowerValue);
                strafe_Left(14,motorPowerValue);
    ...
    private void NavigateToBackstage(PropPostition spikePosition) {
        ...
        switch (spikePosition) {
            case RIGHT:
                ...
                telemetry.update();
                backward(17, motorPowerValue);
                strafe_Left(10,motorPowerValue);
                backward(20.5F, motorPowerValue);
            ...
            case CENTER:
                ...
                sleep(turnSleepSecond);
                backward(17, motorPowerValue);  // this might need to change since the motor specs has been corrected

                strafe_Left(10.5F,motorPowerValue);
                backward(17, motorPowerValue);  // this might need to change since the motor specs has been corrected
                telemetry.addLine("Ready for arm");
                telemetry.update();
                arm_backstage(armPowerValue, arm_tic_value);
            ...
            case LEFT:
                ...
                strafe_Left(7F,motorPowerValue);
                telemetry.addLine("Ready for arm");
                telemetry.update();
                arm_backstage(armPowerValue, arm_tic_value);
                break;
            ...
            case LEFT:
                ...
                switch (parkLocation1){
                    case IN:
                        ...
                        telemetry.update();
                        strafe_Left(32, motorPowerValue); //move away from the backstage

                        break;
                    case OUT:
                        telemetry.addLine("Park OUT");
                        telemetry.update();
                        strafe_Right(36, motorPowerValue); //move away from the backstage
                        break;
            ...
            case CENTER:
                ...
                switch (parkLocation1){
                    case IN:
                        telemetry.addLine("Park IN");
                        telemetry.update();
                        strafe_Left(24, motorPowerValue); //move away from the backstage

                        break;
                    case OUT:
                        telemetry.addLine("Park OUT");
                        telemetry.update();
                        strafe_Right(20, motorPowerValue); //move away from the backstage
                        break;
            ...
            case RIGHT:
                ...
                switch (parkLocation1){
                    case IN:
                        telemetry.addLine("Park IN");
                        telemetry.update();
                        strafe_Left(15, motorPowerValue); //move away from the backstage
                        break;
                    case OUT:
                        telemetry.addLine("Park OUT");
                        telemetry.update();
